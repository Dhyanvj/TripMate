...
const AddExpenseModal = ({ isOpen, onClose, tripId, currentUserId }: AddExpenseModalProps) => {
  ...
  // Initialize participants with current user when modal opens
  useEffect(() => {
    if (isOpen && currentUserId && participants.length === 0) {
      setParticipants([currentUserId]);
    }
  }, [isOpen, currentUserId, participants.length]);
  ...
  const handleSubmit = () => {
    ...
    // Now paidBy can be any group member, addedBy is still the current user
    const expenseData = {
      description,
  ...
  // Get trip member by user ID
  const getTripMemberByUserId = (userId: number) => {
    return tripMembers?.find(member => member.user?.id === userId);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter expense details and select who to split with. Each participant will pay an equal share.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-3">
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              You can add expenses paid by any trip member.
            </p>
          </div>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-0">
          ...
          >
            {addExpenseMutation.isPending ? "Adding..." : (
